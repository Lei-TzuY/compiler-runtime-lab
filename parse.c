...
static Type *declspec(Token **rest, Token *tok) {
    ...
    while (is_decl_start(tok)) {
        ...
        if (equal(tok, "union")) {
            ...
            if (!equal(tok, "{")) {
                if (!tag_name) error_at(tok->loc, "expected union tag or body");
                for (StructTag *st = struct_tags; st; st = st->next)
                    if (!strcmp(st->name, tag_name)) { ty = st->ty; break; }
                if (!ty) error_at(tok->loc, "unknown union: %s", tag_name);
                continue;
            }
            tok = skip(tok, "{");
            ...
            uty->size = align_up(size, align);
            uty->align = align;
            uty->members = head.next;

            if (tag_name) {
                StructTag *st = calloc(1, sizeof(StructTag));
                st->name = tag_name; st->ty = uty;
                st->next = struct_tags; struct_tags = st;
            }
            ty = uty;
            continue;
        }
        ...
        if (equal(tok, "struct")) {
            ...
            if (!equal(tok, "{")) {
                if (!tag_name)
                    error_at(tok->loc, "expected struct tag or body");
                for (StructTag *st = struct_tags; st; st = st->next)
                    if (!strcmp(st->name, tag_name)) { ty = st->ty; break; }
                if (!ty) error_at(tok->loc, "unknown struct: %s", tag_name);
                continue;
            }
            tok = skip(tok, "{");
            ...
            sty->size = align_up(offset, align);
            sty->align = align;
            sty->members = head.next;

            if (tag_name) {
                StructTag *st = calloc(1, sizeof(StructTag));
                st->name = tag_name; st->ty = sty;
                st->next = struct_tags; struct_tags = st;
            }
            ty = sty;
            continue;
